...
// ... Interface ve socialPlatforms tanımları aynı kalıyor ...
...
async function getTeamMembers(): Promise<TeamMemberForPage[]> {
  // Veritabanı sorgusu aynı kalabilir
...
// Rol Hiyerarşisi ve Başlıkları (Aynı kalıyor)
const roleHierarchy: { [key: string]: number } = {
  'Kurucu': 1,
  'Mod Geliştiricisi': 2,
  'SFX/VFX': 3,
  'Çevirmen': 4,
  'Seslendirme Sanatçısı': 5,
};

const roleDisplayTitles: { [key:string]: string } = {
    'Kurucu': 'Kurucu',
    'Mod Geliştiricisi': 'Mod Geliştiricileri',
    'SFX/VFX': 'SFX / VFX',
    'Çevirmen': 'Çevirmenler',
    'Seslendirme Sanatçısı': 'Seslendirme Sanatçıları',
    'Diğerleri': 'Değerli Ekip Üyeleri'
};


// --- YENİ YARDIMCI FONKSİYON: En Yüksek Rolü Bul ---
// Bir üyenin virgülle ayrılmış rollerini alıp en öncelikli olanı döndürür.
function getHighestPriorityRole(siteRoleString: string | null): string {
    if (!siteRoleString) {
        return 'Diğerleri';
    }

    const roles = siteRoleString.split(',').map(role => role.trim());
    
    let highestRole = 'Diğerleri';
    let highestPriority = 999; // Başlangıçta en düşük öncelik

    for (const role of roles) {
        const currentPriority = roleHierarchy[role];
        // Eğer rol hiyerarşide tanımlıysa ve mevcut en yüksek öncelikten daha yüksekse
        if (currentPriority && currentPriority < highestPriority) {
            highestPriority = currentPriority;
            highestRole = role;
        }
    }
    
    // Eğer rollerin hiçbiri hiyerarşide yoksa, ilk rolü veya 'Diğerleri'ni döndür
    if (highestRole === 'Diğerleri' && roles.length > 0) {
        return roles[0];
    }
...
export default async function KadromuzPage() {
  ...
  // --- GRUPLAMA MANTIĞI GÜNCELLENDİ ---
  const groupedMembers = teamMembers.reduce((acc, member) => {
    // Her üye için en baskın rolü bul
    const highestRole = getHighestPriorityRole(member.siteRole);
    
    // Üyeyi, en baskın rolünün grubuna ekle
    if (!acc[highestRole]) {
      acc[highestRole] = [];
    }
    acc[highestRole].push(member);
    return acc;
  }, {} as Record<string, TeamMemberForPage[]>);

  // Grupları sıralama mantığı aynı kalıyor
  const sortedGroupKeys = Object.keys(groupedMembers).sort((a, b) => {
    const priorityA = roleHierarchy[a] || 99;
    const priorityB = roleHierarchy[b] || 99;
  ...
  return (
    <div style={{ backgroundColor: '#08060D' }} className="text-white min-h-screen py-16 md:py-20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        {/* Sayfa başlığı aynı kalıyor */}
        <div className="text-center mb-14 md:mb-16">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight bg-clip-text bg-gradient-to-r from-purple-400 to-indigo-500">
            Ekibimizle Tanışın
          </h1>
        ...
        {teamMembers.length === 0 ? (
          ...
          <div className="space-y-16">
            {sortedGroupKeys.map(role => (
              <section key={role}>
                <h2 className="text-3xl font-bold text-center mb-10 border-b-2 border-indigo-500/30 pb-4">
                  {roleDisplayTitles[role] || role}
                </h2>
                  ...
                  {groupedMembers[role].map((member) => {
                    ...
                    return (
                      <div 
                        key={member.id} 
                        className={cn(
                          "group relative flex flex-col p-4 rounded-2xl transition-all duration-300 ease-out overflow-hidden w-full max-w-64",
                      ...
                      >
                        ...
                        <div className="z-10">
                            <div className="flex flex-col items-center text-center mb-4">
                                ...
                                {/* Kartın üzerinde tüm roller görünür */}
                          ...
                          {member.bio && (<p className="text-sm text-gray-400 leading-relaxed text-center mb-5">"{member.bio}"</p>)}
